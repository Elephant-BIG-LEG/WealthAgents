...
class FinancialAnalyzer:
    ...
    def analyze_financial_data(self, company_name: str, financial_data: Dict[str, Any]) -
                             > Dict[str, Any]:
        """
        分析公司财务数据
        ...
        try:
            ...
            for metric_name, calculator in self.financial_metrics.items():
                try:
                    results['financial_metrics'][metric_name] = calculator(financial_data)
                except Exception as e:
                    logger.warning(f"计算指标 {metric_name} 时出错: {str(e)}")
                    results['financial_metrics'][metric_name] = None
            
            # 评估财务健康状况
            results['financial_health'] = self._evaluate_financial_health(results['financial_metrics'])
            
            # 分析增长趋势
            if 'historical_data' in financial_data:
                results['growth_trends'] = self._analyze_growth_trends(financial_data['historical_data'])
    ...
    def analyze_company_from_knowledge_base(self, company_name: str, knowledge_base) -
                                          > Dict[str, Any]:
        """
        从公司知识库中分析公司财务状况
        ...
        try:
            ...
            for query in financial_queries:
                results = knowledge_base.query_company_knowledge(company_name, query, top_k=3)
            ...
            # 从合并的文本中提取财务数据
            extracted_data = self.extract_financial_data_from_text(combined_text)
            
            # 使用提取的数据进行财务分析
            if extracted_data:
                analysis_results = self.analyze_financial_data(company_name, extracted_data)
                return {
                    'status': 'success',
    ...
    def generate_financial_report(self, company_name: str, analysis_results: Dict[str, Any]) -
                                > str:
        """
        生成格式化的财务分析报告
    ...
    def _analyze_growth_trends(self, historical_data: List[Dict[str, Any]]) -
        ...
        # 分析收入趋势
        revenues = [d.get('revenue') for d in historical_data if 'revenue' in d]
        if len(revenues) > 1:
            growth = (revenues[-1] - revenues[0]) / revenues[0] if revenues[0] != 0 else 0
            if growth > 0.3:
                trends['收入趋势'] = '快速增长'
            elif growth > 0:
                trends['收入趋势'] = '稳定增长'
        ...
        if len(profits) > 1:
            growth = (profits[-1] - profits[0]) / profits[0] if profits[0] != 0 else 0
            if growth > 0.3:
                trends['利润趋势'] = '快速增长'
            elif growth > 0:
                trends['利润趋势'] = '稳定增长'
    ...
    def _extract_value_by_keywords(self, text: str, keywords: List[str]) -
                                 > Optional[float]:
        """根据关键词从文本中提取数值"""
        for keyword in keywords:
            ...
            for sentence in sentences:
                if keyword in sentence:
                    # 尝试提取数字
                    numbers = re.findall(r'\d+(?:\.\d+)?(?:[万亿万千百]?)', sentence)
                    if numbers:
                        # 处理中国数字单位（万亿、亿、万）
